...
def _fertiliser_slope(df=pd.DataFrame) -> pd.DataFrame:
    ...
    df["net_import_qty"] = df.import_quantity - df.export_quantity
...
def shortage_analysis(export_cut_russia_belarus: float = 1) -> None:
    ...
    fao_nutrients = (
        pd.read_csv(f"{config.paths.raw_data}/fertiliser_nutrients_fao.csv")
        .pipe(clean_fao)
        .loc[
            :,
            [
    ...
    summary_df["variable"] = [
        ...
        "import demand by rest of world excluding g20",
        "imports by g20 as % of total",
        "shortage",
        "import demand gap (%)",
        "g20 imports used in agriculture (%)",
    ...
    for fertiliser in fao_nutrients.fertiliser.unique():
        ...
        rus_bel_export = df[
            df.iso_code.isin(["BLR", "RUS"])
        ].export_quantity.sum()  # exports from russia and Belarus
        g20_import = df[
            df.iso_code.isin(utils.g20)
        ].import_quantity.sum()  # imports from g20

        exports_available = (
            total_export - (rus_bel_export * export_cut_russia_belarus) - g20_import
        )  # exports available w/out russia/belarus and after g20 demand is met
        demand_rest_of_world = (
            total_import - g20_import
        )  # import demand by rest of world excluding g20

        g20_import_pct = (
            g20_import / total_import
        ) * 100  # imports by g20 as % of total
...
def flourish_slope_africa(df: pd.DataFrame, fertiliser_list: list, name: str) -> None:
    ...
    df = (
        df.pipe(_fertiliser_slope)
        .loc[
            (df.fertiliser.isin(fertiliser_list))
            & (df.continent == "Africa")
            & (df.net_import_qty >= 0)
            & (df.change != 0),
            [
...
def dependence_maps(df: pd.DataFrame) -> None:
    ...
    for fertiliser in df.fertiliser.unique():
        fertiliser_df = (
            df.loc[
                df.fertiliser == fertiliser,
                ["country", "iso_code", "continent", "fertiliser", "dependence"],
            ]
        ...
        fertiliser_df.to_csv(
            f"{config.paths.output}/dependence_map_{fertiliser}.csv", index=False
        )
...
def update_charts() -> None:
    ...
    dependence_maps(
        fao_nutrients
    )  # create/update dependence maps for nitrogen phosphate and potash
