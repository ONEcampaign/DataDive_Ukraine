...
from scripts.trade_impact import get_latest_prices_data, get_yearly_prices_data, get_spending, _calc_mean_prices




# fertilisers keys = pink sheet name, values = FAO name
fertilisers = {'Phosphate rock':'Phosphate rock',
...
def clean_raw_fao(df:pd.DataFrame) ->pd.DataFrame:
    ...
    df = (df[['Area', 'Element', 'Item', 'Year', 'Unit', 'Value']]
          .pivot(index=['Area', 'Item', 'Unit', 'Element'], columns=['Year'], values='Value')
          .reset_index())
    df['value'] = (df[2018] + df[2019])/2 #calculate mean of 2019 and 2020
    df = (df.drop(columns=[2018, 2019])
          ...
          .replace({'China, Taiwan Province of':'Taiwan', 'China, mainland':'China'})) #fix naming convention for China for country_converter mapping
...
def fertilizer_pipeline():
    ...
    df['net_import_qty'] = df.import_quantity - df.export_quantity ################## what about NAN values? Assume 0?
    ...
    df = df.pipe(get_spending, ['net_import_qty'], [('latest',latest_price), ('pre_crisis', means)])
...
def flourish_slope_africa(df:pd.DataFrame, fertiliser:str):
    ...
    return (df[(df.fertiliser == fertiliser)&(df.continent == 'Africa')]
          .dropna(subset=['value_net_import_qty_latest', 'value_net_import_qty_pre_crisis'])
          [['iso_code', 'country', 'fertiliser', 'pink_sheet_commodity', 'value_net_import_qty_latest', 'value_net_import_qty_pre_crisis']])
